...
ensure_pacman_keyring() {
  if run_root pacman-key --list-keys >/dev/null 2>&1; then
    return
  fi

  warn "Pacman keyring is not initialized. Initializing now."
...
bootstrap_user_if_needed() {
  ...
  if [ -z "$target_user" ]; then
    read -r -p "Enter username to create for Arch XRDP setup: " target_user
  fi
  ...
  if grep -Eq '^#\s*%wheel ALL=\(ALL:ALL\) ALL' /etc/sudoers; then
...
install_xrdp_packages() {
  if pacman -Si xrdp >/dev/null 2>&1 && pacman -Si xorgxrdp >/dev/null 2>&1; then
    info "Installing xrdp/xorgxrdp from pacman repos"
    run_root pacman -S --noconfirm --needed xrdp xorgxrdp
  else
    info "Installing xrdp/xorgxrdp from AUR"
    install_yay_if_missing
...
main() {
  ...
  ensure_pacman_keyring
  info "Installing Arch base packages + XFCE"
  run_root pacman -Syu --noconfirm --needed base-devel git xfce4 xfce4-goodies
  ...
  if [ "${WSL_UI_TEMP_SUDOERS:-0}" = "1" ]; then
    ...
    info "If this was first-time Arch setup, restart distro once for new default user: wsl --terminate archlinux"
  fi
