...
ensure_password_set() {
  ...
  if [ "$status" = "NP" ] || [ "$status" = "L" ] || [ "$status" = "LK" ]; then
    warn "Your Linux user does not have a usable password. XRDP and Win-KeX login need one."
...
main() {
  ...
  info "Installing Win-KeX and XRDP packages"
  run_root apt update
  run_root env DEBIAN_FRONTEND=noninteractive apt install -y kali-win-kex xrdp xorgxrdp
  ...
  info "Setup complete."
  info "WSL UI / manual XRDP: mstsc -> localhost:$target_port"
  info "Win-KeX ESM (recommended Kali mode):"
  printf '  kex --esm --sound\n'
  info "If ESM cannot connect in NAT/ARM cases, use:"
  printf '  kex --esm --ip --sound\n'
